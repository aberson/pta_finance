...
**Baseline:** `main` at `a421c94` on 2026-09-23. **Planning issue:** #85. **Umbrella:** #86. **Status:** READY for build after #72. Plan review and plan wrap both returned READY on 2026-09-23; build issues #87–#92 are synced. Phase 9 still owns Steps 38–49, and its deferred receipt picker owns Step 50; this plan uses Steps 51–56. **Build entry gate:** Phase 9 Step 38 / issue #72 must repair the existing CI browser failure before Step 51 starts; the 2026-09-23 CI run for `a421c94` still fails in `lint-type-test` because Chromium is absent.
...
- `plan.md` reserves Steps 38–49 for automatic receipt filling; `documentation/receipt-autofill-plan.md:160` reserves Step 50 for a later receipt picker. Main CI's browser gate is already red and Phase 9 Step 38 owns its repair. This plan must not silently claim that unrelated CI defect is fixed.
...
| Existing test gates | Main CI browser gate is already red; a full local suite needs Firestore emulator and native extras. | Complete Phase 9 Step 38 / #72 before Step 51; record exact gate evidence during Step 55 without mislabeling a subset as full. |
...
**Build handoff:** Complete Phase 9 Step 38 / #72 and verify CI first. Then run `/build-phase --plan documentation/non-reimbursement-action-queue-plan.md` for automated Steps 51–55. Stop at Step 56 for attended private-archive acceptance.
