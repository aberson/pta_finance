...
### 2026-09-06 payment-confirmation-lanes amendment

Schema-v2 anchors add two explicit payment lanes and withdraw payment authority from every other
link. `payment_links` names one normalized archived Message-ID from a configured payment operator
and binds each selected ticket to the SHA-256 of its exact confirmation reference; the mail must
parse under one of two strict grammars — the generated single-ticket sentence
(`Your $AMOUNT reimbursement has been approved and sent by Zelle.` followed by
`Zelle confirmation: REFERENCE`, bound to the configured signoff and, for single bindings, the
ticket's email context) or the known one-, two-, or three-block Zelle wire layout — and the parsed
blocks must match the bindings one-to-one with every amount equal to its already-approved ticket's
reviewed-effective total. A malformed, partial, duplicated, wrong-sender, out-of-scope, or
ancestry-conflicting group is quarantined as a whole without touching any ticket.
`operator_payments` records an audited payment where no email exists (`record_payment: true`, one
exact selector, ISO date, exact amount, raw reference, nonblank audit note) and settles only an
already-approved ticket whose total matches exactly. Ordinary thread anchors and direct links
continue to account for correspondence but never authorize payment; a ticket may sit in only one
payment lane and a reference in only one lane. Schema-v1 anchors keep their historical reducer,
including the companion clarification/receipt events a v1 payment mail produced, so an exact v2
upgrade replays byte-for-byte. Landed on `main` 2026-09-06 as `01f7bff` (merge `aa9b1b1`).

Known fail-closed gaps recorded at landing (next fix step; none can mutate a bundle): settlement
blanks the `email_context` the generated-single grammar is bound to, so a bundle-held context with no
live operator review cannot replay on the next refresh; the frozen v1 lane records against the
source total while the loader validates the reviewed-effective total; and v1 non-Zelle or
discrepancy histories cannot replay under v2 despite the reproducibility promise above.
...
Completion evidence (updated 2026-09-06, payment-confirmation lanes): 950 collected tests, full
suite green on Windows with the `slides` extra installed, strict mypy, Ruff lint/format, and the
identity check all passed on `aa9b1b1`; a 24-agent adversarial landing review confirmed no path
records payment without an exact configured-operator confirmation and no abort mutates the bundle.
The real private refresh replayed byte-for-byte across repeated dry runs after landing.
