...
## Maintenance — mixed claim closure and receipt coverage — shipped 2026-09-23

**Status: COMPLETE for the shipped code.** This is a focused maintenance change; Phase 9's
automatic receipt producer remains planned.

### What was built

- A settled reimbursement claim can retain both paid approved lines and declined lines. The
  validator requires a positive amount in each group plus payment date and confirmation.
- The closed-case appendix labels mixed outcomes **Paid + declined**, displays their resolution,
  and keeps them out of the active action index without changing any line decision or amount.
- Report commands print validated receipt-link coverage, making unlinked items visible after
  every run. Recent private receipts were manually linked and the private report was rebuilt.

### Files changed

| File | Change |
|---|---|
| `pta_finance/reimbursement_report.py` | Validate settled mixed outcomes; return receipt-link count |
| `pta_finance/cli.py` | Print linked and unlinked item counts |
| `pta_finance/reports/templates/reimbursement_queue.html.j2` | Show mixed closure and resolution |
| `tests/test_reimbursement_report.py`, `tests/test_reimbursement_cli.py`, `tests/test_receipt_viewer.py` | Cover mixed closure and count output |
| `README.md`, `docs/receipt-viewer.md`, `CLAUDE.md` | Refresh operator and current-state guidance |

Validation: 822 passed, 2 skipped (824 collected) on the suite excluding the six
Firestore-emulator modules and two native-PDF modules. The full suite cannot collect without
`FIRESTORE_EMULATOR_HOST`; the native run was stopped before completion. Focused reimbursement
